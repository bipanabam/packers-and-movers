...
const Hero = () => {
  return (
    <section id="hero" className="w-full px-4 md:px-8 py-16">
        <div className="flex flex-col md:flex-row items-center md:items-start justify-center gap-10 md:gap-2">
                ...
                <div className="flex flex-col items-start gap-5 w-full">
                    ...
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 w-4/5">
                        ...
                        <div className="flex items-center gap-3 text-secondary">
                            ...
                            <div className="flex flex-col text-[16px]">
                                <span>Insured</span>
                                <span className="text-neutral text-xs">all moves covered</span>
                            </div>
                        </div>
                    </div>
            ...
            <div className="w-full md:flex-1 relative">
                <div className="relative w-full h-87.5 md:h-137.5 rounded-2xl overflow-hidden">
                    <Image
                        src="/assets/hero.png"
                        alt="Professional packers and movers loading furniture in Kathmandu"
                        fill
                ...
                <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-2 text-secondary bg-white/30 rounded-md px-2.5 py-1 backdrop-blur-2xl shadow-sm hover:scale-105" aria-hidden="true">
        ...
        {/* Trust strip */}
        <div className="mt-10 pt-4 border-t border-primary/20 grid grid-cols-2 md:grid-cols-4 gap-y-3">
            {[
                { icon: <Star size={18} fill="#FF7A00" className="text-secondary" />, label: "4.8 rating", sub: "500+ reviews" },
                { icon: <ShieldCheck size={18} className="text-secondary" />, label: "Background checked", sub: "verified movers" },
                { icon: <Lock size={18} className="text-secondary" />, label: "Fully insured", sub: "all services" },
                { icon: <MapPin size={18} className="text-secondary" />, label: "Kathmandu Valley", sub: "local experts" },
            ].map((item) => (
                <div key={item.label} className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-md bg-secondary/20 flex items-center justify-center shrink-0">
                        {item.icon}
                    </div>
                    <div className="flex flex-col">
                        <span className="text-xs font-medium text-foreground leading-tight">{item.label}</span>
                        <span className="text-[10px] text-foreground/50 leading-tight">{item.sub}</span>
                    </div>
                </div>
            ))}
        </div>
